...
export async function onRequestPost(context) {
  ...
  const rateLimitResponse = await applyRateLimit(
    ...
    20,
    3600,
    'Too many cover letter requests. Please try again in an hour.'
  );
  ...
  const coverLetter = sanitizePlainText(claudeData.content?.[0]?.text?.trim() || '', 16000);
  ...
  return json({ cover_letter: coverLetter, scans_remaining: tokenData.scans_remaining }, 200, headers);
}
